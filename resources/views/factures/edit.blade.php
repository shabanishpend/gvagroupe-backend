            ...
            <form class="box" action="{{ route('factures.update', [$website]) }}" method="POST" id="pdf_form">
                ...
                <div class="d-flex">
                    <div class="form-group mt-2 mr-2 d-flex flex-column">
                        ...
                        <select class="form-control select2 select" data-toggle="select2" id="subscription_type" name="subscription_type">
                            <option @if($facture->subscription_type == "") selected @endif value="">Sélectionner un souscription</option>
                            <option @if($facture->subscription_type == "monthly") selected @endif value="monthly">Mensuel</option>
                            <option @if($facture->subscription_type == "3_months") selected @endif value="3_months">3 Mois</option>
                            <option @if($facture->subscription_type == "6_months") selected @endif value="6_months">6 Mois</option>
                            <option @if($facture->subscription_type == "yearly") selected @endif value="yearly">Annuel</option>
                        </select>
                    </div>
                    <div class="form-group mt-2 mr-2 d-flex flex-column">
                        <label class="" for="subscription_type">Période du</label>
                        <input style="min-width: 112px;" class="font_bold form-control" value="{{ $facture->subscription_start_date }}" type="date" placeholder="Écrire date" name="subscription_start_date" id="subscription_start_date"  />
                    </div>
                    <div class="form-group mt-2 d-flex flex-column">
                        <label class="" for="subscription_type">Jusqu'au</label>
                        <input style="min-width: 112px;" class="font_bold form-control" value="{{ $facture->subscription_end_date }}" type="date" placeholder="Écrire date" name="subscription_end_date" id="subscription_end_date"  />
                    </div>
                </div>
                @endif
                    ...
                    <table style="width: 100%;">
                        ...
                        <tr class="item">
                            <td>
                                ...
                                @if($website == 'maflotte' && $facture->subscription_start_date)<span style="font-size: 14px;">Pour la période du <span id="subscription_start_date_text">{{ date('d.m.Y', strtotime($facture->subscription_start_date)) }}</span> au <span id="subscription_end_date_text">{{ date('d.m.Y', strtotime($facture->subscription_end_date)) }}</span></span> @endif
                            </td>
